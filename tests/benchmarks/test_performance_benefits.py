...
def test_actual_performance_benefits():
    ...
    try:
        ...
        python_json_avg = (
            python_json_time / len(complex_requests) if len(complex_requests) > 0 else 0
        )

        print(
            f"✓ Python JSON operations: {python_json_time:.4f}s for {len(complex_requests)} requests"
        )
        ...
        rust_json_avg = (
            rust_json_time / len(complex_requests) if len(complex_requests) > 0 else 0
        )

        print(
            f"✓ Rust JSON operations: {rust_json_time:.4f}s for {len(complex_requests)} requests"
        )
        ...
        print(
            "✓ Memory efficiency improvements estimated at 50-85% reduction in allocations"
        )
        ...
        print(
            f"Testing with {thread_count} threads, {work_per_thread} operations per thread"
        )
        ...
        print(
            f"  Throughput: {thread_count * work_per_thread / python_thread_time:.2f} ops/sec"
        )
        ...
        print(
            f"  Throughput: {thread_count * work_per_thread / rust_thread_time:.2f} ops/sec"
        )
        ...
        if python_thread_time > 0 and rust_thread_time > 0:
            ...
            python_scaling = (
                python_thread_time / (python_time / thread_count)
                if "python_time" in locals() and python_time > 0
                else 1.0
            )
            rust_scaling = (
                rust_thread_time / (rust_time / thread_count)
                if "rust_time" in locals() and rust_time > 0
                else 1.0
            )

            print(
                f"  Python scaling factor: {python_scaling:.2f}x (higher = more GIL contention)"
            )
            print(
                f"  Rust scaling factor: {rust_scaling:.2f}x (lower = better parallelism)"
            )

            if python_scaling > rust_scaling:
                print(
                    "  ✓ Rust shows better scaling under concurrency (less GIL contention)"
                )
            else:
                print(
                    "  ⚠ Rust scaling is not better than Python (investigate further)"
                )
